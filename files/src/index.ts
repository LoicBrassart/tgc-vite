...
dotenv.config();
const { FILES_PORT } = process.env;
if (!FILES_PORT) throw new Error("Missing essential env variables!");
...
app.listen(FILES_PORT, async () => {
  console.log(`Example app listening on port ${FILES_PORT}`);
});
